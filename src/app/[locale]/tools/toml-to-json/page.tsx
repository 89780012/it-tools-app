...
export default function TomlToJsonPage() {
  ...
  const convertTomlToJson = () => {
    ...
    try {
      ...
      setError(t("tools.toml-to-json.invalid_toml"))
      setOutput("")
    }
  }
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div>
        <h1 className="text-3xl font-bold">{t("tools.toml-to-json.name")}</h1>
        <p className="text-muted-foreground mt-2">
          {t("tools.toml-to-json.description")}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>
              {t("tools.toml-to-json.input_title")}
            </CardTitle>
            <CardDescription>
              {t("tools.toml-to-json.input_desc")}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Textarea
              ...
              placeholder={t("tools.toml-to-json.placeholder")}
            ...
            <div className="flex gap-2">
              <Button onClick={convertTomlToJson} className="flex-1">
                {t("common.convert")}
              </Button>
        ...
        <Card>
          <CardHeader>
            <CardTitle>
              {t("tools.toml-to-json.output_title")}
            </CardTitle>
            <CardDescription>
              {t("tools.toml-to-json.output_desc")}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <Textarea
              value={output}
              readOnly
              placeholder={t("tools.toml-to-json.output_placeholder")}
      ...
      {/* SEO 优化内容 */}
      <ToolSEOSection toolId="toml-to-json" />
    </div>
  )
}
