...
export default function FirewallCheatsheetTool() {
  const t = useTranslations('tools.firewall-cheatsheet')
  ...
  const filteredCommands = useMemo(() => {
    return filterCommands(allCommands, searchQuery, activeCategory)
  ...
  const handleCopy = async (cmd: string) => {
    try {
      ...
      toast({ title: t('copy_success') })
      setTimeout(() => setCopiedCommand(null), 2000)
    } catch {
      toast({ title: common('copy_failed'), variant: 'destructive' })
    }
  }

  const totalCommands = useMemo(() => countCommands(filteredCommands), [filteredCommands])
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
        ...
        <CardContent className="pt-6 space-y-4">
          ...
          {searchQuery && (
            <p className="text-sm text-muted-foreground">
              {totalCommands === 0 ? t('no_results') : t('results_count', { count: totalCommands })}
            </p>
          )}
        </CardContent>
      </Card>
      ...
      <ToolSEOSection toolId="firewall-cheatsheet" />
    </div>
  )
}
