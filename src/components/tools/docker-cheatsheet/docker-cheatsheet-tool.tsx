...
export default function DockerCheatsheetTool() {
  const t = useTranslations('tools.docker-cheatsheet')
  ...
  const totalCommands = useMemo(() => {
    return countCommands(filteredCommands)
  }, [filteredCommands])
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
        ...
        <CardContent className="pt-6 space-y-4">
          ...
          {searchQuery && (
            <p className="text-sm text-muted-foreground">
              {totalCommands === 0
                ? t('no_results')
                : t('results_count', { count: totalCommands })}
            </p>
          )}
        </CardContent>
      </Card>
      ...
      {/* SEO Section */}
      <ToolSEOSection toolId="docker-cheatsheet" />
    </div>
  )
}
